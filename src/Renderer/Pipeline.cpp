...
namespace FirstEngine {
    namespace Renderer {
        ...
        void Pipeline::CreateRenderPass() {
            ...
            colorAttachment.format = m_Renderer->GetSwapchain()->GetImageFormat();
        ...
        void Pipeline::CreateGraphicsPipeline() {
            // TODO: Load actual shader files using ShaderLoader
            // Example:
            // auto vertShaderCode = FirstEngine::Shader::ShaderLoader::LoadShader("shaders/vert.spv");
            // auto fragShaderCode = FirstEngine::Shader::ShaderLoader::LoadShader("shaders/frag.spv");
            //
            // For now, using empty shaders will cause pipeline creation to fail.
            // You need to compile GLSL shaders to SPIR-V and load them here.
            
            std::vector<char> vertShaderCode; // Empty - must be replaced with actual shader
            std::vector<char> fragShaderCode;  // Empty - must be replaced with actual shader
            ...
            viewport.x = 0.0f;
            viewport.y = 0.0f;
            viewport.width = (float)m_Renderer->GetSwapchain()->GetExtent().width;
            viewport.height = (float)m_Renderer->GetSwapchain()->GetExtent().height;
            viewport.minDepth = 0.0f;
            viewport.maxDepth = 1.0f;

            VkRect2D scissor{};
            scissor.offset = {0, 0};
            scissor.extent = m_Renderer->GetSwapchain()->GetExtent();
            ...
            if (vkCreateGraphicsPipelines(m_Renderer->GetDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_GraphicsPipeline) != VK_SUCCESS) {
        ...
        void Pipeline::CreateFramebuffers() {
            const auto& swapchainImageViews = m_Renderer->GetSwapchain()->GetImageViews();
