    ...
    namespace Renderer {
        ...
        bool PipelineBuilder::BuildFromConfig(const PipelineConfig& config, FrameGraph& frameGraph) {
            ...
            for (const auto& resourceConfig : config.resources) {
                ...
                desc.type = ParseResourceType(resourceConfig.type);
                desc.name = resourceConfig.name;
                desc.width = resourceConfig.width;
                desc.height = resourceConfig.height;
                desc.format = ParseFormat(resourceConfig.format);
                desc.size = resourceConfig.size;
                desc.hasDepth = (resourceConfig.format == "D32_SFLOAT" || resourceConfig.format == "D24_UNORM_S8_UINT");
            ...
            for (const auto& passConfig : config.passes) {
                ...
                if (passConfig.type == "geometry") {
                    node->SetExecuteCallback([passConfig](RHI::ICommandBuffer* cmd, FrameGraphBuilder& builder) {
