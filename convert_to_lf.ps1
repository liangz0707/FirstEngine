# PowerShell script to convert all source files from CRLF to LF
# This ensures consistent line endings across the project

Write-Host "Converting line endings from CRLF to LF for all source files..." -ForegroundColor Green
    ...
    foreach ($file in $files) {
        try {
            ...
            if ($content -match "`r`n") {
                ...
                # Write back with UTF8 encoding (no BOM) and LF line endings
...
Write-Host "  1. Update .gitattributes to use LF for all text files" -ForegroundColor Yellow
Write-Host "  2. Run: git add --renormalize ." -ForegroundColor Yellow
